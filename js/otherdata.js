function get_user_info(){
    var  dictPost  =  {"plugin":"user_model","action":"get_user_data"};
    console.log(dictPost);
    var Jr=[];
    $.ajax({
        type: "POST",
        url: "/plugin/",
        data: dictPost,
        success: function(response){
            console.log(response);
            Jr = JSON.parse(response);
            if(Jr['success']==true) {
                // load info
                if(Jr.avatar==null){
                    document.getElementById('side-head').setAttribute( 'src', 'data:image/png;base64,' );
                }
                else {
                    document.getElementById('side-head').setAttribute( 'src', Jr.avatar );
                }
                document.getElementById('user-email').innerHTML=Jr.email
            }
            else {
                Materialize.toast(Jr['error'], 3000, 'rounded');
            }

            if(Jr.username=null){
                if(Jr.username.length>0){
                    document.getElementById('this_is_a_user_name').innerHTML=Jr.username
                }
            }
            //this is for the current user name


        }

    });
}

function get_others_info() {
    var  dictPost  =  {"plugin":"user_model","action":"get_others_data"};
    console.log(dictPost);
    var Jr=[];
    $.ajax({
        type: "POST",
        url: "/plugin/",
        data: dictPost,
        success: function(response){
            console.log(response);
            Jr = JSON.parse(response);
            if(Jr['success']==true) {
                // load info
                if(Jr.avatar==null){
                    document.getElementById('user-head').setAttribute( 'src', 'data:image/png;base64,' );
                }
                else {
                    document.getElementById('user-head').setAttribute( 'src', Jr.avatar );
                }

                if(Jr.education=null){
                    if(Jr.education.length>0){
                        document.getElementById('edu').innerHTML=Jr.education
                    }
                }

                if(Jr.username=null){
                    if(Jr.username.length>0){
                        document.getElementById('user_name').innerHTML=Jr.username
                    }
                }

                if(Jr.describe=null){
                    if(Jr.describe.length>0){
                        document.getElementById('describe').innerHTML=Jr.describe
                    }
                }

                if(Jr.major=null){
                    if(Jr.major.length>0){
                        document.getElementById('describe').innerHTML=Jr.major
                    }
                }
                 if(Jr.username=null){
                if(Jr.username.length>0){
                    document.getElementById('this_is_another_user_name').innerHTML=Jr.username
                }
            }

            }
            else {
                Materialize.toast(Jr['error'], 3000, 'rounded');
            }
        }

    });

function get_all(){
    get_user_info();
        get_others_info_();
}

function applyFriend(){
    return function(){
    var id1=document.getElementById("this_is_a_user_name");
    var id2=document.getElementById("this_is_another_user_name");
    var dictPost={"plugin":"profile","action":"apply_friend","user_id1":id1,"user_id2":id2};
        console.log(dictPost);
        var jsonResp=[];
        $.ajax({
                type:"POST",
                url:"/plugin/",
                data:dictPost,
                success:function(response){
                        console.log(response);
                        jsonResp=JSON.parse(response);
                        if(jsonResp['success']==true){
                                alert("Successfully submitted!");
                        }
                        else{
                                Materialize.toast(jsonResp['error'],2500,'rounded');
                        }
                }
        });
    }
}